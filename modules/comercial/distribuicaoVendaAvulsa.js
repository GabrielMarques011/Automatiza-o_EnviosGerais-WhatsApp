...
const WHATSAPP_GROUP_ID = process.env.WHATSAPP_GROUP_ID_COMERCIAL; //process.env.WHATSAPP_GROUP_ID_TESTE
const TIMEOUT = 15000;
...
export async function distribuicaoVendaAvulsa(tokenArg) {
  ...
  try {
    ...
    // 1) listar tickets (su_ticket) assunto 540
    const ticketsRaw = await paginarPost("/su_ticket", { qtype: "id_assunto", query: "540", oper: "=" });

    // 2) filtrar por su_status EP ou N e montar mapa ticketId -> id_responsavel_tecnico
    const ticketsMap = {};
    ...
    console.log(`📌 Tickets (assunto 540) com su_status EP/N: ${Object.keys(ticketsMap).length}`);
    if (!Object.keys(ticketsMap).length) return;

    // 3) listar OSS (su_oss_chamado) assunto 540 e filtrar status = "A"
    const ossRaw = await paginarPost("/su_oss_chamado", { qtype: "id_assunto", query: "540", oper: "=" });
